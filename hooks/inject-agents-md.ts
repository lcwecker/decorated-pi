...
interface AgentsMdRuntime {
    discovered: Set<string>;
    pendingPaths: Map<string, string>;
    cwd: string;
}

function createRuntime(): AgentsMdRuntime {
    return { discovered: new Set(), pendingPaths: new Map(), cwd: "" };
}

function resetRuntime(runtime: AgentsMdRuntime): void {
    ...
    for (const entry of branch.slice(start)) {
        ...
        for (const p of paths) {
            if (typeof p === "string" && p.trim()) {
                runtime.discovered.add(normalizeAbsPath(ctx.cwd, p));
            }
        }
    }
}
...
function findNewAgents(
    ...
    while (true) {
        ...
        for (const name of AGENTS_NAMES) {
            ...
            if (existsSync(agentsPath) && !runtime.discovered.has(agentsPath)) {
                try {
                    const content = readFileSync(agentsPath, "utf-8");
                    runtime.discovered.add(agentsPath);
...
export const __subdirAgentsTest = { createRuntime, restoreFromBranch, findNewAgents, isInsideSkillDir };
...
export function createInjectAgentsMdModule(): Module {
        ...
        hooks: {
            ...
            tool_call: [
                (event) => {
                    ...
                    runtime.pendingPaths.set(event.toolCallId, path);
                },
            ],
            tool_result: [
                (event, ctx, pi) => {
                    const path = runtime.pendingPaths.get(event.toolCallId);
                    runtime.pendingPaths.delete(event.toolCallId);
                    if (!path || !event.content || !Array.isArray(event.content))
                        return;
                    const cwd = ctx.cwd ?? runtime.cwd;
                    const agents = findNewAgents(runtime, path, cwd);
